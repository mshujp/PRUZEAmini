...
ImageLayout calculateImageLayout(
    ...
    Image::ImageFit fit)
{
    ...
    if (fit == Image::ImageFit::STRETCH) return layout;

    const float scale = fit == Image::ImageFit::CONTAIN
...
class ImageLGFX final : public Image::ImageData
{
private:
    struct PngDecodeState
    {
        ImageLGFX* image = nullptr;
    ...
    static bool readJpegSize(const uint8_t* data, uint32_t size, uint16_t& outputWidth, uint16_t& outputHeight)
    {
...
public:
    ImageLGFX() : sprite(nullptr) {}
    ImageLGFX(const ImageLGFX&) = delete;
    ImageLGFX& operator=(const ImageLGFX&) = delete;

    bool create(uint16_t outputWidth, uint16_t outputHeight)
    {
    ...
    bool decodeJpeg(const uint8_t* data, uint32_t size, Image::ImageFit fit)
    {
    ...
    bool decodePng(const uint8_t* data, uint32_t size, Image::ImageFit fit)
    {
...
private:
    ~ImageLGFX() override
    {
        if (psramBuffer == nullptr) return;
        sprite.deleteSprite();
        free(psramBuffer);
    }
};

Image::ImageData* load(const uint8_t* data, uint32_t size, uint16_t outputWidth, uint16_t outputHeight, Image::ImageFit fit, bool png)
{
    if (data == nullptr || size == 0 || outputWidth == 0 || outputHeight == 0) return nullptr;

    auto* image = new (std::nothrow) ImageLGFX();
    if (image == nullptr) return nullptr;

    const bool decoded = image->create(outputWidth, outputHeight) &&
        (png ? image->decodePng(data, size, fit) : image->decodeJpeg(data, size, fit));
    if (!decoded)
    {
        image->close();
        return nullptr;
...
Image::ImageData* PRUZEAmini::Image::loadJpeg(const uint8_t* jpegData, uint32_t jpegSize, uint16_t outputWidth, uint16_t outputHeight, ImageFit fit)
{
    return load(jpegData, jpegSize, outputWidth, outputHeight, fit, false);
}

Image::ImageData* PRUZEAmini::Image::loadPng(const uint8_t* pngData, uint32_t pngSize, uint16_t outputWidth, uint16_t outputHeight, ImageFit fit)
{
    return load(pngData, pngSize, outputWidth, outputHeight, fit, true);
}
