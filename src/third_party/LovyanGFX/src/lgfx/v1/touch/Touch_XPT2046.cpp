 ...
 inline namespace v1
 {
  ...
  uint_fast8_t Touch_XPT2046::getTouchRaw(touch_point_t* __restrict tp, uint_fast8_t count)
  {
    ...
    else
    {
      ...
      if (_cfg.pin_cs > -1) lgfx::gpio_lo(_cfg.pin_cs);
      spi::readBytes(_cfg.spi_host, data, 57);
      if (_cfg.pin_cs > -1) lgfx::gpio_hi(_cfg.pin_cs);
    ...
    for (size_t j = 0; j < 7; ++j)
    {
      ...
      int z = 0x3200 + y - x
            + (((d[3] << 8 | d[4])
              - (d[7] << 8 | d[8])) >> 1);
      if (x > 128 && x <= 3968) xt[ix++] = x;
      if (y > 128 && y <= 3968) yt[iy++] = y;
      if (z > 0) zt[iz++] = z;
    }
    if (ix < 3 || iy < 3 || iz < 3) return 0;
